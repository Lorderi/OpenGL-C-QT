...
// парсим файл и записываем все нужные данные в массивы структур (см. models,
// что есть в какой структуре)
void parse_file(Data_array* data_array, File_data* file_information,
                FILE* file) {
  //сюда будут помещаться строки (считаю, что все файлы корректны, поэтому
  //размер строки 256 символов - хватит на все, сколькими бы числами не
  //задавались координаты (разряды) - если будет больше 256 символов - тупо не
  //влезет по точности в double)
  char buffer[STR_SIZE] = {0};
  // устанавливаем курсор в начало файла, чтобы файловые потоки не передергивать
  // постоянно
